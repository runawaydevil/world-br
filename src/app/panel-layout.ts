...
export class PanelLayoutManager implements AppModule {
  ...
  renderLayout(): void {
    this.ctx.container.innerHTML = `
        ...
        <div class="header-left">
          <div class="variant-switcher">${(() => {
            ...
            <a href="${vHref('br', 'https://br.worldmonitor.app')}"
               class="variant-option ${SITE_VARIANT === 'br' ? 'active' : ''}"
               data-variant="br"
               ${vTarget('br')}
               title="${t('header.brasil')}${SITE_VARIANT === 'br' ? ` ${t('common.currentVariant')}` : ''}">
              <span class="variant-icon">🇧🇷</span>
              <span class="variant-label">${t('header.brasil')}</span>
            </a>
          ...
          <span class="logo">Grupo Murad</span>
      ...
      <footer class="app-footer">Desenvolvido pelo Grupo Murad - ${new Date().getFullYear()}</footer>
    `;
