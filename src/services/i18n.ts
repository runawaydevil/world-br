...
function applyDocumentDirection(lang: string): void {
  ...
  const langAttr = base === 'zh' ? 'zh-CN' : base === 'pt' ? 'pt-BR' : base;
...
export async function initI18n(): Promise<void> {
  ...
  let effectiveLanguage = await ensureLanguageLoaded(i18next.language || 'en');
  if (SITE_VARIANT === 'br') {
    await ensureLanguageLoaded('pt');
    ...
    await i18next.changeLanguage(effectiveLanguage);
  }

  applyDocumentDirection(i18next.language || effectiveLanguage);
}
